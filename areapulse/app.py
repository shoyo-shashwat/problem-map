...
from ai_routes import ai_bp, init_ai_tables
from ai_assistant import analyze_report
...
AREA_COORDS = {
    ...
    'Pushp Vihar': [28.5190, 77.2130], 'Deoli': [28.4980, 77.2160],
...
def ai_assistant_page():
    return render_template('ai_assistant.html', current_user=session.get('user'))
...
def report():
    try:
        ...
        ai_result = analyze_report(area, desc, user)

        if ai_result['is_spam']:
            return jsonify({
                'status':     'spam',
                'reason':     ai_result['spam_reason'],
                'confidence': ai_result['confidence'],
            }), 200

        corrected   = ai_result['corrected_description']
        tag         = ai_result['suggested_tag'] or auto_tag(corrected)
        corrections = ai_result['spelling_corrections']
        ...
        issue_id = insert_issue(area, corrected, tag, user, lat, lng, image_data, severity, landmark, contact)
        add_points(user, 10)

        nearby = get_nearby_ngos(lat, lng, tag=tag, limit=3) if lat and lng else []
...
def user_stats():
    ...
    pts   = stats['points']
    stats['level'] = ('Civic Hero' if pts >= 500 else 'Contributor' if pts >= 200
                      else 'Active Member' if pts >= 50 else 'Newbie')
    stats['name'] = name
