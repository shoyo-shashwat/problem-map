...
DB_PATH = os.environ.get('DB_PATH', 'areapulse.db')
...
def seed_real_issues():
    ...
    if count > 0:
        ...
        return
    seeds = [
        ('Rohini',        'Massive pothole on Rohini Sector 3 main road near D-mall, causing daily accidents',          'pothole',     'Arav Sharma',   'open',      14, 28.7041, 77.1025),
        ('Karol Bagh',    'Sewage overflow on Arya Samaj Road blocking pedestrian path, foul smell since 3 days',       'sewage',      'Priya Mehta',   'verified',   9, 28.6514, 77.1907),
        ...
        ('Chandni Chowk', 'Water pipe burst near Fatehpuri Masjid flooding entire lane, shops flooded',                 'water',       'Vikram Sethi',  'escalated', 31, 28.6506, 77.2303),
        ...
        ('Saket',         'Open manhole on Press Enclave Road near Select Citywalk dangerous for cyclists',             'sewage',      'Neha Agarwal',  'open',      25, 28.5244, 77.2090),
        ('Vasant Kunj',   'Broken water pipeline at B9 DDA Flats, residents without water for 2 days',                 'water',       'Karan Malhotra','verified',  16, 28.5200, 77.1590),
        ('Shahdara',      'Illegal garbage dumping site near Shahdara drain, burning waste causing toxic smoke',         'garbage',     'Pooja Yadav',   'open',      20, 28.6706, 77.2944),
        ('Mayur Vihar',   'Traffic signal at Mayur Vihar Phase 1 metro not working for 5 days',                         'traffic',     'Amit Joshi',    'open',      13, 28.6090, 77.2944),
        ('Janakpuri',     'Frequent power cuts in Janakpuri Block C, 4-6 hours outage daily',                           'electricity', 'Sunita Rawat',  'open',       8, 28.6219, 77.0878),
        ...
        ('Model Town',    'Broken footpath with exposed iron rods near Model Town Park, elderly fell',                  'pothole',     'Kavita Sharma', 'verified',  12, 28.7167, 77.1900),
        ('Geeta Colony',  'Open electrical wires hanging low over road near Geeta Colony flyover',                      'electricity', 'Rajan Sood',    'open',      34, 28.6590, 77.2780),
        ...
        ('Burari',        'Garbage truck not coming to Burari Sector 4 for 6 days, residents burning waste',            'garbage',     'Harish Negi',   'open',      23, 28.7470, 77.2100),
        ('Uttam Nagar',   'Deep pothole crater on Uttam Nagar West road, 50cm deep, bus got stuck yesterday',          'pothole',     'Sachin Tyagi',  'open',      41, 28.6219, 77.0560),
    ]
    ts = time.time()
    for s in seeds:
        area, desc, tag, user, status, upvotes, lat, lng = s
        db.execute(
            "INSERT INTO issues (area,description,tag,timestamp,upvotes,status,user,lat,lng,severity) VALUES (?,?,?,?,?,?,?,?,?,'medium')",
    ...
    for name, pts in [('Delhi Civic Watch', 320), ('NGO Volunteer', 180), ('RWA Rohini', 95)]:
